...
| A shared Linux box, no sudo, rootless docker available | On the remote: `nixsh` (container `nix` from `nixos/nix`, `/nix` in a volume, home bind-mounted) | Native-speed Nix inside the container. Only the package set of `.#<user>-linux-shared` (`config.home.path`, no secrets) goes on PATH; configs stay the remote-bootstrap symlinks so host-side tools keep working. `nixsh update` (daily via `dotfiles-sync.timer`) rebuilds it |
