...
| A shared Linux box, no sudo, rootless docker available | On the remote: `nixsh` (container `nix` from `nixos/nix`, `/nix` in a volume, home bind-mounted) | Native-speed Nix inside the container; home-manager as `.#<user>-linux-shared` (no secrets, no age key on the host) |
