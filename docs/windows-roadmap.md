...
| P9-41 | 実機 cleanup(別 commit、apps.json には影響なし) — HP プリインの Bitdefender Total Security 三点セット(Total Security + VPN + BDContextualMenu、試用切れ) / Poly.PolyLens(HP/Plantronics オーディオ管理、未使用) / Office 2013 デスクトップ版(OneNote 含む、2014 EOL の試用版) — winget Bitdefender Free と競合を避けるため Total を先に削除する必要あり。HP Wolf Security と Vivado は保護(残す) | ⏳ pending(管理者 PowerShell 手動実行) |
