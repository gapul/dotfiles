...
export function SignUpForm() {
  ...
  const signUpMutation = api.auth.register.useMutation({
    onSuccess: () => {
      toast({
        title: '登録成功',
        description: 'アカウントが作成されました。ログインページにリダイレクトしています...',
      })
  ...
  const handleOAuthSignIn = async (provider: 'google' | 'github') => {
    ...
    try {
      ...
      toast({
        title: 'エラー',
        description: `${provider}での登録に失敗しました。`,
        variant: 'destructive',
      })
    } finally {
      setIsLoading(false)
  ...
  const isFormLoading = isLoading || signUpMutation.isLoading
