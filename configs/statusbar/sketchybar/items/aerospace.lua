...
function get_workspaces()
    -- Return static workspace list 1-8 for compatibility
    return {"1", "2", "3", "4", "5", "6", "7", "8"}
end

function get_current_workspace()
    local file = io.popen("/Applications/AeroSpace.app/Contents/MacOS/AeroSpace list-workspaces --focused")
    local result = file:read("*a")
    file:close()

    return parse_string_to_table(result)[1] or "1"
end

function get_monitors()
    local file = io.popen("/Applications/AeroSpace.app/Contents/MacOS/AeroSpace list-monitors | awk '{print $1}'")
    local result = file:read("*a")
    file:close()
...
function get_workspaces_on_monitor(monitor)
    -- Return static workspace list for compatibility
    return {"1", "2", "3", "4", "5", "6", "7", "8"}
end
