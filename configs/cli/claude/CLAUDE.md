...
3. **terminal-browser** — 実Chromium（Electron同梱）なので描画の穴がない。
   ...
   **いまherdrの中では「見せる」用途に使えない**（2026-08-31, herdr 0.8.2）。
   絵を出すには`[experimental] kitty_graphics`が要るが、これを入れると描画が
   タブに閉じず、別タブを表示している間もそのタブの文字の上にページが重なる。
   切っていると今度はペインもプロセスも正常なのに**何も描かれない**。どちらも
   無言で、エラーは出ない。**ユーザーに見せる必要があるものはZenに投げる**。
   `action`での読み取り/操作は絵と無関係に効くので、そこは使える。
