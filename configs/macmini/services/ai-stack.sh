...
# The AI panel is what homeserver's Caddy proxies as tools.gapul.net over the
# tailnet (100.105.135.49:8901), so it has to listen beyond loopback. #445 made
# both uvicorns loopback-only and tools.gapul.net answered 502 from then on
# (found 2026-09-26). The embedding server on :8900 stays on loopback.
AI_PANEL_BIND_ADDR="${AI_PANEL_BIND_ADDR:-0.0.0.0}"
...
while true; do
  ...
  up http://127.0.0.1:8901/ || (nohup "$H/.local/share/venvs/rag-venv/bin/python" -m uvicorn ai_panel:app --host "$AI_PANEL_BIND_ADDR" --port 8901 --app-dir "$H/.local/share/ai-stack" >/tmp/ai_panel.log 2>&1 &)
  sleep 30
done
