...
start_owui(){ container run -d --name open-webui --dns 1.1.1.1 -e OLLAMA_BASE_URL=http://192.168.116.91:11434 -e WEBUI_AUTH=False -e RAG_EMBEDDING_ENGINE=ollama -e HF_HUB_OFFLINE=1 -v $H/.local/share/open-webui:/app/backend/data ghcr.io/open-webui/open-webui:main >/dev/null 2>&1; }
...
while true; do
  ...
  up http://127.0.0.1:8900/health || (nohup $H/.local/share/venvs/rag-venv/bin/python -m uvicorn rag_server:app --host 0.0.0.0 --port 8900 --app-dir $H/.local/share/ai-stack >/tmp/rag_server.log 2>&1 &)
  up http://127.0.0.1:8901/ || (nohup $H/.local/share/venvs/rag-venv/bin/python -m uvicorn ai_panel:app --host 0.0.0.0 --port 8901 --app-dir $H/.local/share/ai-stack >/tmp/ai_panel.log 2>&1 &)
  # コンテナランタイム+各コンテナ
