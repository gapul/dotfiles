...
start_owui(){ container run -d --name open-webui --dns 1.1.1.1 -e OLLAMA_BASE_URL=http://host.container.internal:11434 -e WEBUI_AUTH=True -e RAG_EMBEDDING_ENGINE=ollama -e HF_HUB_OFFLINE=1 -v "$H/.local/share/open-webui:/app/backend/data" ghcr.io/open-webui/open-webui:main >/dev/null 2>&1; }
start_allm(){ container run -d --name anythingllm --dns 1.1.1.1 -e STORAGE_DIR=/app/server/storage \
  -e LLM_PROVIDER=ollama -e OLLAMA_BASE_PATH=http://host.container.internal:11434 -e OLLAMA_MODEL_PREF=gemma4:12b-it-qat -e OLLAMA_MODEL_TOKEN_LIMIT=8192 \
  -e EMBEDDING_ENGINE=generic-openai -e EMBEDDING_BASE_PATH=http://host.container.internal:8900/v1 -e EMBEDDING_MODEL_PREF=ruri-v3-310m -e GENERIC_OPEN_AI_EMBEDDING_API_KEY=sk-none -e EMBEDDING_MODEL_MAX_CHUNK_LENGTH=512 \
  -e VECTOR_DB=lancedb \
  -v "$H/.local/share/anythingllm:/app/server/storage" docker.io/mintplexlabs/anythingllm:latest >/dev/null 2>&1; }
...
while true; do
  ...
  if ! { up http://127.0.0.1:3001/ && up http://127.0.0.1:3000/; }; then
    "$H/.local/share/ai-stack/container_proxy.sh" >/dev/null 2>&1
  fi
