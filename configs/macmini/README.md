...
| Ollama本体 + launchd agent | **nix宣言** | `nix/hosts/macmini.nix`(agentは未ロード=ai-stack.shが起動) |
| ラッパーCLI(transcribe/tts/ask等) | 本リポ管理 | `configs/macmini/bin/` → `~/.local/bin/` |
...
- **量子化×日本語**: VLM/LLMの4bitは日本語生成を壊しうる(EMNLP2024で査読済)。Gemma QAT / Ruri は日本語堅牢。
