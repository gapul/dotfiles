# macOS ユーザー名変更ガイド（2025年6月更新版）

このドキュメントは、macOS 15.5環境でユーザー名を「Yuki」から「yuki」に変更する手順を説明します。
...
**現象：**
- ディレクトリ名は既に `/Users/yuki` （小文字）
- しかしユーザー名は「Yuki」（大文字Y）のまま
- これによりNixなどのツールで ownership の不整合が発生

**根本原因：**
- macOSの登録ユーザー名：「Yuki」（大文字Y）
- ホームディレクトリ名：「yuki」（小文字y）
- この不一致により権限チェックでエラーが発生
...
# Nix設定の確認
cd ~/dotfiles && nix flake check --no-build 2>&1 | head -5
```
...
# 現在の設定を記録（2025年6月版）
{
    ...
    echo -e "\n=== ディレクトリ情報 ==="
    ls -la /Users/ | grep -E "(Yuki|yuki)"
    
    echo -e "\n=== Directory Services情報 ==="
    sudo dscl . -read /Users/Yuki
    
    echo -e "\n=== SSH設定確認 ==="
    ls -la ~/.ssh/ 2>/dev/null || echo "SSH設定なし"
    
    echo -e "\n=== 開発環境確認 ==="
    which nix && nix --version || echo "Nix未インストール"
    which brew && brew --version | head -1 || echo "Homebrew未インストール"
    
} > ~/Desktop/pre_username_change_$(date +%Y%m%d_%H%M%S).txt
```

### Phase 2: 一時管理者ユーザーの作成

#### 2.1 管理者ユーザーの作成（macOS 15.5対応）

**GUI操作（macOS 15.5）：**
1. **システム設定** (System Settings) を開く
2. **ユーザとグループ** (Users & Groups) をクリック
3. 右上の **編集** ボタンをクリックして認証
4. **ユーザまたはグループを追加** をクリック
5. 以下の設定で新規ユーザーを作成：
   - **種類**: 管理者
   - **氏名**: `Temporary Admin`
   - **アカウント名**: `tempAdmin`
...
**注意：macOS 15.5では設定項目の配置が変更されています**
...
#### 3.2 ユーザー名変更の実行（macOS 15.5対応）

```bash
# 1. Yukiユーザーの現在の設定を確認・保存
sudo dscl . -read /Users/Yuki > ~/Desktop/yuki_user_backup_$(date +%Y%m%d_%H%M%S).txt

# 2. 現在の状況確認（重要：ホームディレクトリは既に/Users/yuki）
echo "=== 変更前状況確認 ==="
ls -la /Users/ | grep -E "(Yuki|yuki)"
sudo dscl . -read /Users/Yuki NFSHomeDirectory

# 3. レコード名の変更（メイン作業）
echo "レコード名を Yuki → yuki に変更中..."
sudo dscl . -change /Users/Yuki RecordName Yuki yuki

# 4. 実名も統一（オプション）
echo "実名の統一..."
sudo dscl . -change /Users/yuki RealName "Yuki KAWASHIMA" "yuki"
...
# 6. 重要なディレクトリの権限確認
echo "SSH設定の権限確認..."
sudo chown -R yuki:staff /Users/yuki/.ssh
...
#### 7.1 一時管理者の削除（macOS 15.5対応）

**GUI操作（macOS 15.5）：**
1. **システム設定** > **ユーザとグループ** を開く
2. 右上の **編集** ボタンをクリックして認証
3. tempAdminユーザーを選択
4. **ユーザまたはグループを削除** をクリック
5. **ホームフォルダを削除** を選択して削除実行
...
**作成日**: 2025年6月18日  
**更新日**: 2025年6月20日（macOS 15.5対応）  
**対象システム**: macOS 15.5+ (Apple Silicon/Intel)  
**対象ユーザー**: Yuki → yuki  
...
### 2025年6月20日
- macOS 15.5環境での動作確認・更新
- 現在のシステム状況に基づく手順の最適化
- GUI操作手順をmacOS 15.5に対応
- 既存SSH設定の考慮
- Nix Flake check手順の最新化
- just コマンド統合
...
```bash
# ユーザー名: Yuki（大文字Y）
# ホームディレクトリ: /Users/yuki（小文字y）
# NFSHomeDirectory: /Users/yuki ✓
# ディレクトリ所有者: Yuki ✓
# SSH設定: 正常存在 ✓
# Nix設定: 動作確認済み ✓
```
...
### 変更の必要性
- **主要作業**: Directory Services のレコード名変更のみ
- **ディレクトリ移動**: 不要（既に/Users/yuki）
- **権限修正**: 最小限（SSH等の重要ファイルのみ）
- **リスク**: 低（レコード名変更のみのため）

### 変更による効果予測
- Nix $HOME ownership warning完全解消
- 開発ツール互換性向上
- Unix系システムとの一貫性確保