# Generated by compose2nix from /opt/stacks/dawarich/compose.yaml on the old Docker host,
# then hand-adjusted: ./ paths became /var/lib/homelab/dawarich, /mnt/jellyfin-media became
...

{
  ...
  virtualisation.oci-containers.containers."dawarich_db" = {
    ...
    log-driver = "journald";
    extraOptions = [
      "--health-cmd=pg_isready -U dawarich -d dawarich_production"
      "--health-interval=10s"
      "--health-retries=5"
      "--health-start-period=30s"
      "--health-timeout=10s"
  ...
  systemd.services."podman-volume-dawarich_dawarich_db_data" = {
    ...
    script = ''
      podman volume inspect dawarich_dawarich_db_data || podman volume create dawarich_dawarich_db_data --driver=local --opt=device=/srv/dawarich/postgres --opt=o=bind --opt=type=none
    '';
    partOf = [ "podman-compose-dawarich-root.target" ];
    wantedBy = [ "podman-compose-dawarich-root.target" ];
  };
  ...
  systemd.services."podman-volume-dawarich_dawarich_storage" = {
    ...
    script = ''
      podman volume inspect dawarich_dawarich_storage || podman volume create dawarich_dawarich_storage --driver=local --opt=device=/srv/dawarich/storage --opt=o=bind --opt=type=none
    '';
    partOf = [ "podman-compose-dawarich-root.target" ];
    wantedBy = [ "podman-compose-dawarich-root.target" ];
  };
